...
sys.path.append("../")
from hgDecompose.optimizedGDecompose import GrDecompose
...
if (args.paramrand>0):
    from hgDecompose.utils import get_random_graph
    import random
    ...
    if args.paramrand == 1:
        ...
        parameter = {'n': 14, 'm': 10, 'card_max': 2}
        ...
        print(parameter)
        print(' seed: ',seed)
        Hg = get_random_graph(n = n, m = m, seed = seed)
        print(Hg)
        Hg_cpy = deepcopy(Hg) 
        hgDecompose = GrDecompose()
        hgDecompose.Graph_top_down(Hg, s=args.param_s, verbose=True)
        ...
        for parameter in param_list:
            ...
            for i  in range(0, N):
                ...
                hgDecompose = GrDecompose()
                hgDecompose.Graph_top_down(Hg, s=args.param_s, verbose=False)
                core_compared = hgDecompose.core
...
H = Hypergraph(scenes)
hgDecompose = GrDecompose()
hgDecompose.Graph_top_down(H, s=args.param_s, verbose=True)
core_compared = hgDecompose.core
print('Topdown: ',sorted(core_compared.items()))
